...
so you see Yoshimi already configured exactly as you like, with patches loaded
and part destinations set.

To make it easier to position patch changes in a running MIDI file, there is a
new option to report the time these take to load.
...
instrument can seem to be changed instantly, retaining the note tails of the
previous one.


V 1.4.0 Diamond Dove
...
for data entry of values, the NRPN is of itself the entire command. With a
   suitable sequencer like Rosegarden, you start with a two byte value, then
   only need to enter single byte CCs to change the setting.  See
...
Some people don't like our splash window - Boo!  You can now disable it in
settings {mutter}{mutter}
