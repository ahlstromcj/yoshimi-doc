...
\label{subsubsec:concepts_terms_part}
   ...
   Since around version 1.3.5, this limit has been raised to 64.
...
%  in \sectionref{subsubsec:menu_instrument_save},
   or through the \textbf{Bank} window.
%  in \sectionref{subsubsec:menu_instrument_show_banks}.
...
\label{subsubsec:concepts_terms_patch_set}
   ...
   See \sectionref{subsec:concepts_patch_sets}, which has a deeper treatment of
   this term.
...
\label{subsubsec:concepts_terms_preset}
   ...
   program is selected via a "program-change" message.  In \textsl{Yoshimi}, a
...
\subsection{Concepts / Patch Sets}
\label{subsec:concepts_patch_sets}
   ...
   \begin{itemize}
      \item \texttt{.xmz}
         A full \textsl{Yoshimi} configuration. Everything.  It's format is
         either XML or compressed XML, as explained below.
      ...
      \item \texttt{.config}
         ...
         \texttt{\$HOME/.config/yoshimi} directory.  These files contain
         ...
         reduced in size, and its "doctype" is no longer "ZynAddSubFX".  The
         config is always going to be specific to one machine and working
         modes, so no one will want to copy it across even to another
         \textsl{Yoshimi} environment.  Recent patch sets are now no longer
         stored in the main \texttt{.config} file, but in a new
         \texttt{.history} file.  This file is now a much reduced common
         settings -- interfaces, sample rate -- file.  It is a single file that
         every instance can read, but only the first one can write.
         \texttt{.config} hasn't yet been separated from \texttt{.instance(n)}
         and all files are still per instance, as the \textsl{Yoshimi} team
         haven't had time to work out exactly how to manage common files and
         memory locations for those that should be shared.  The
         \texttt{.config} file is saved only when the user explicitly calls for
         it to be saved.
      \item \texttt{.state}
      ...
      \item \texttt{.xpz}
         ...
         \textbf{C} and \textbf{P} user-interface buttons associated with many
      ...
      \item \texttt{.instance(n)}
         ...
         These are totally independent files.
      \item \texttt{.history}
         A new feature of the \textsl{Yoshimi} configuration.
         ...
         The \texttt{.history} file is save only upon a normal exit, as it is
         comparatively unimportant.
      \item \texttt{.banks}
         A new feature of the \textsl{Yoshimi} configuration.
         Currently each \textsl{Yoshimi} instance takes its own copy of the
         actual files as it starts up.  However, they can all save, delete, or
         rename the actual files without talking to the other instances, so one
         can move a file in one instance, and then try (and fail) to access it
         from another.
      \item \texttt{.windows}
         A new feature of the \textsl{Yoshimi} configuration.
         It saves the current layout of windows for reinstantiation at the next
...
\subsubsection{Concepts / Patch Sets / All}
\label{subsec:concepts_patch_sets_all}
...
\subsubsection{Concepts / Patch Set / Management}
\label{subsec:concepts_patch_set_management}

   The 'state' file consists of the entire setup from basic configuration to
   currently loaded instrument sets.  However, on investigating some JACK
