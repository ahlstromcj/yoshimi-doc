...
\label{subsubsec:concepts_terms_cent}
   ...
   The \textbf{cent} is a logarithmic unit of measure used for musical
   ...
   intervals in different tuning systems, and in fact the interval of one
   cent is much too small to
   be heard between successive notes.
...
\label{subsec:concepts_banks_and_roots}
   ...
   controls are contained in a tab in the main Settings window, and take
   immediate effect.
   ...
   MIDI only sees banks in the \textsl{current} root dir, but all banks are
...
\label{paragraph:concepts_midi_nrpn_vector_control}
   ...
   \sectionref{subsection:nrpns_midi_nrpn_vector_control}.
