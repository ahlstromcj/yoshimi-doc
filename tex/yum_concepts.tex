...
\label{subsubsec:concepts_terms_frame}
   ...
   commonly integers, 24 bit integers (low byte ignored), and short integers.
   Less commonly it may be floats or the weird 24-bit long integers.  Are these
   packed or low-aligned (top byte ignored)? It's assumed they are low-aligned,
   but we don't know anyone who has such a card in order to prove it. As a
   matter of interest, the only ALSA format \textsl{Yoshimi} doesn't support is
   float; again, no way to test it.
...
\label{subsec:concepts_alsa_versus_jack}
   ...
   integers (16 bit), 24 bit integers (low byte ignored), and short integers.
   ...
   in order to prove it.  As a matter of interest the only ALSA format
