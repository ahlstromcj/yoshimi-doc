...
\label{subsubsec:concepts_terms_preset}
   ...
   Note that the number of preset directories that can be set is limited to
   1000 (unlike roots and banks).
...
\label{subsubsec:concepts_basics_randomness}
   ...
   There is now the possibility to add 'naturalising' random pitch and velocity
   elements to a part. This is found in the part-edit window. The settings are
   saved in patch sets, and also in \textsl{yoshimi} format instruments files.
...
\label{subsubsec:concepts_midi_messages}
   ...
   \begin{itemize}
      \item \textbf{Bank Change (0 or 32)} These are intechangable and do
...
\label{subsec:concepts_instances}
   ...
   \begin{quotation}
       ...
       too). Exernal instances are where Yoshimi is restarted from the desktop
       ...
       \textbf{Power user:}
          ...
          Rosegarden. It sends it's audio to JACK.
   ...
   from the main instance's AddSynth part 3 to instance 3's kite item 6 AddSynth.
