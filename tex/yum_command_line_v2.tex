...
\label{sec:command_line}
   ...
   Also, the whole of vector control, and much of MIDI-learn, is also exposed
   to the command line.
...
%  command line.
   ...
   Since V 1.5.10 you no longer need \textbf{ty}pe \textbf{re}verb and instead can directly enter:
...
\texttt{RUN [s]} &
   ...
   This script can also include time delays with a WAIT command.\\
\texttt{..} &
...
%      \begin{itemize}
%         \item GRoup [s]. Disables harmonic amplitude randomness of voices with a
%              common oscillator (ON, {other}).
%         \item BAndwidth [n]. Modifes relative fine detune of voices.
%         \item REsonance ... Enter the AddSynth resonance context.
%         \item VOice ... Enter Addsynth voice context.
%      \end{itemize}
...
\texttt{Common controls plus:} & \\
\texttt{Type [s]} &
   Modulator type (OFF, Morph, Ring, Phase, Frequency, Pulse width). \\
\texttt{Source [[s]/[n]]} &
...
\label{paragraph:command_line_mlearn}
   ...
   The commands for the second form are the following values for the [s]
   parameter:
...
\paragraph{EFfects Controls}
\label{paragraph:command_line_effects}
...
\label{subsec:command_line_vector_command_list}
   ...
   Although \textbf{Vector} is a top-level control, the list can be called from any level with '? ve'.
