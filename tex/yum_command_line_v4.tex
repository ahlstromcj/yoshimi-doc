...
\label{sec:command_line}
   ...
   These text commands can also be put into a script file, and that script
   can then be run with full error checking.
   ...
   For full details of NRPNs see \sectionref{subsection:nrpns_midi_nrpn_basics}

   In a command line environment, all the 'running' commands are
...
\label{subsec:command_line_command_depth}
   ...
   The levels higher up the tree are minimised:
   ...
   show if a patchset or instrument is loaded that was previously saved
   with altered effects controls.
   ...
   These two special level-movement commands can also be put on the front
   of any other command.  Starting where we were before:
...
%  Also note that the space after the ".." is optional.
   ...
   command-line exit, the last commands will be available on the next run
   of \textsl{Yoshimi}.

   The command-line now has formal methods of opening, selecting and
   closing additional instances.

   When loading external files from the command line, there is an
   alternative to entering the full name if \textsl{Yoshimi} has already
   seen this file and it is in the history list. In this situation one
   enters '@' followed by the list number.
   ...
   If any configuration settings are changed, either at the command-line or
   in the graphical user-interface, one will be given a warning when
...
\label{subsec:command_line_command_scripts}
   ...
   The script routine honours any normal abbreviations.
...
\label{subsec:command_line_other_command_tables}
   ...
   Also, from any level, \textbf{? ?} will show the top level one.
...
\texttt{List [s]} &
   List current information (optional 's = topic). \\
\texttt{STop} &
...
\texttt{EXit [s]} &
   Tidy up and close \textsl{Yoshimi} (if 'y' to the prompt), 'FOrce'
   instant with no checks and exit value of 16 \\
\texttt{RUN [s]} &
   Execute a simple named script file, first setting top level.
   This script can also include time delays with a \textbf{WAIT} command.\\
\texttt{RUNLocal [s]} &
   As above but starting from the current context level.\\
\texttt{..} &
...
\label{subsec:command_line_mlearn}
   ...
   \begin{itemize}
      ...
      \item \texttt{CC [n]} will set the incoming controller value that will
         be recognised.  This command can re-order the list.
...
\label{subsec:command_line_instrument_group_list}
   ...
   Repeating the command with one of these names will then search the entire
