...
\end{figure}
   ...
   First we show the manual edit that both the modified types were taken from, as shown in \figureref{fig:addsynth_resonance_edit}.

\begin{figure}[H]
   \centering
   \includegraphics[scale=0.8]{2.0/ResonanceSource.png}
   \caption{ADDsynth/PADsynth Resonance Edit}
   \label{fig:addsynth_resonance_edit}
\end{figure}

   Click either the \textbf{InterpPk S} button (smooth interpolation) or the \textbf{InterpPk L} button (linear interpolation). \textsl{Yoshimi} will interpolate automatically between the peaks drawn, as shown in \figureref{fig:addsynth_resonance_interpolated}, which shows
   \textsl{smooth interpolation}.

\begin{figure}[H]
   \centering
   \includegraphics[scale=0.8]{2.0/ResonanceInterpolated.png}
...
\begin{figure}[H]
   \centering
   \includegraphics[scale=0.8]{2.0/ResonanceSmoothed.png}
...
These parameters are:
   ...
   \itempar{Depth}{LFO!depth}
   \index{LFO!amount}
   LFO Depth.  Also called "LFO Amount". Uniquely, this control can be set greater than full depth, and the LFO envelope becomes re-entrant. A sine shape becomes two humps, as if rectified, triangle becomes two, etc.
   ...
   Turns the LFO frequency control into a ratio control related to the incoming MIDI timecode. New in \textsl{Yoshimi} V2.0
...
%   LFO Type (or Shape, or Function).
   ...
   \texttt{RSQu} maintains the freqency (doesn't skip any changes) but
   alternates between the depth control setting and a random level from that
   to the maximum depth.

   \texttt{RSQd} also maintains the freqency but alternates between the
...
\begin{figure}[H]
   \centering
   \includegraphics[scale=0.8]{2.0/AddSynthAmpEnv.png}
...
\begin{figure}[H]
   \centering
   \includegraphics[scale=0.8]{2.0/AddSynthAmpEnvFree.png}
...
%  \includegraphics[scale=0.75]{zyn/formant_filter.png}
   \includegraphics[scale=1.0]{2.0/AddSynthFormantFilter.png}
