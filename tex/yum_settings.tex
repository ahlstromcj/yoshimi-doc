...
\paragraph{Title Bars}
\label{paragraph:stock_settings_elements_title_bars}
   ...
   Oscillator Editor, one also sees the number of the oscillator one is
   editing.

\paragraph{Color Coding}
\label{paragraph:stock_settings_elements_color_coding}
...
\paragraph{Knobs}
\label{paragraph:stock_settings_elements_knobs}
...
\paragraph{Windows}
\label{paragraph:stock_settings_elements_windows}
...
\label{paragraph:stock_settings_elements_automation}
   ...
   \begin{enumber}
      \item textbf{NRPNs}
      \item textbf{ZynAddSubFX controls}
      \item textbf{Independent part control}
      \item textbf{16, 32 or 64 parts}
      \item textbf{Vector Control}
      \item textbf{Direct part stereo audio output}
   \end{enumber}
   ...
   System \& Insertion Effect controls are fully supported, with extensions
...
\label{subsec:filter_settings}
   ...
   See section~\ref{subsubsec:filter_parameters_user_interface}
   on page~\pageref{subsubsec:filter_parameters_user_interface}
...
\label{subsubsec:filter_type}
   ...
   offer are shown in figure~\ref{fig:basic_filter_types} on
   page~\pageref{fig:basic_filter_types}, with
...
%  on page~\pageref{fig:2_pole_vs_8_pole_filter}.
   ...
   \texttt{AnalogFilter::computefiltercoefs()} sets the coefficients (depending
   on the filter type), and \\
...
\end{figure}
   ...
   above.
%  figure~\ref{fig:filter_parameters_subpanel} on
%  page~\pageref{fig:filter_parameters_subpanel}.
...
\end{figure}
   ...
   figure~\ref{fig:filter_categories_dropdown} on
   page~\pageref{fig:filter_categories_dropdown}.)
...
\end{enumber}
   ...
   and band-pass.
   See the dropdown element in figure~\ref{fig:filter_type_dropdown} on
   page~\pageref{fig:filter_type_dropdown}.
...
\end{figure}
   ...
   This dropdown
   element is shown in
   figure~\ref{fig:filter_stage_dropdown} on
   page~\pageref{fig:filter_stage_dropdown}.
...
\end{figure}
   ...
   figure~\ref{fig:basic_parameters_lfo} on
   page~\pageref{fig:basic_parameters_lfo}.)
...
\label{subsubsec:lfo_function}
   ...
   figure~\ref{fig:types_of_lfo} on
   page~\pageref{fig:types_of_lfo}.
...
\end{figure}
   ...
   frequency or LFO amplitude changes by this parameter. In
   figure~\ref{fig:randomness_in_lfo} on
   page~\pageref{fig:randomness_in_lfo}
   are shown some examples of randomness and how it changes the shape of a
   triangle LFO.
...
\end{figure}
   ...
   Figure~\ref{fig:amplitude_lfo} shows an example of an LFO stock sub-panel.
...
These parameters are:
   ...
   LFO Stretch. In the image in
   figure~\ref{fig:amplitude_lfo} on
   page~\pageref{fig:amplitude_lfo}, the LFO stretch is set to zero,
...
\end{figure}
   ...
   figure~\ref{fig:lfo_function_type_dropdown} on
   page~\pageref{fig:lfo_function_type_dropdown}.
   ...
   For reference,
   figure~\ref{fig:filter_lfo} on
   page~\pageref{fig:filter_lfo} shows the LFO sub-panel for a filter, and
   figure~\ref{fig:frequency_lfo} on 
   page~\pageref{fig:frequency_lfo}
...
\end{figure}
   ...
   LFO Type.
      ...
      TODO Figure: bottom-panel/instrument-edit/ADD/lfo-function-type.jpg
...
\end{figure}
   ...
   This envelope, shown in
   Figure~\ref{fig:asr_envelope_depiction} on
   page~\pageref{fig:asr_envelope_depiction},
   is an ASR envelope.
...
\end{figure}
   ...
   TO BE DESCRIBED ELSEWHERE, such as next section (followed by freemode
   section)
...
\label{subsubsec:freemode_envelope_settings}
   ...
   Figure~\ref{fig:amplitude_envelope_freemode}
   on page~\pageref{fig:amplitude_envelope_freemode}
...
\end{figure}
   ...
   \ref{fig:amplitude_envelope_freemode}
...
\label{subsubsec:envelope_settings_for_frequency}
   ...
   Figure~\ref{fig:asr_envelope_depiction} on
   page~\pageref{fig:asr_envelope_depiction}
   depicts the stages of these envelopes.
   The dotted line represents the real pitch of the sound without the envelope.
...
\label{paragraph:formant_parameters}
   ...
   Vowel "Clearness".
   How much the vowels are kept "clear".
...
\index{subsubsec:clipboard!copy}
   ...
   Figure~\ref{fig:copy_to_clipboard} on
   page~\pageref{fig:copy_to_clipboard} 
   show an example of the copying dialog for the clipboard.
...
\index{subsubsec:clipboard!paste}
   ...
   Figure~\ref{fig:paste_to_clipboard} on
   page~\pageref{fig:paste_to_clipboard} 
   show an example of the pasting dialog for the clipboard.
...
\end{figure}
   ...
   Paste from preset.
   Prrovides a way to specify the preset to which this data should be
   copied.
   TODO:  How do you know the correct name for the preset?
