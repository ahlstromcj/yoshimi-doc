...
\label{subsubsec:stock_settings_elements_sliders}
   ...
   happen if you click on the track with the right button.  If on top of the
...
\end{enumber}
   ...
   are a bit like human voices.  Here is Paul Nasca himself (mildly edited)
   on how to undertand the format filter:

   \begin{quotation}
      The easiest way to understand the formants/vowel is to imagine at first
      that each vowel contains a single formant.

      In that case, each vowel consists, in sequence, of a band-pass filter
      (with Q/frequency/amplitude). These vowels are arranged on a line
      (sequence), and the space between them is controlled by the \textbf{Strch}
      (lower right). This sequence could be used to create some simple words
      and the current position in that line is controlled by the filter
      LFO/Envelope.

      So, for example, you can define two vowels like: "vowel\_1" which has the
      formant at 1000Hz and the "vowel\_2" with the formant at 2000Hz and
      define a simple raising envelope. You should get a sliding bandpass
      filter between these (eg: you get a bandpass filter which rises in the
      frequency from 1000Hz to 2000Hz).
      ...
      using the second formant. The formants are bandpass filters, in parallel,
      as you've guessed.

      I preferred for a vowel to have several formants (than the other way around)
