...
\label{sec:effects}
   ...
   specified way. More exactly, the only input data for an effect
...
\label{subsec:effects_edit_dynfilter}
   ...
   parameters dynamically, dependent on the input and current time.
   Frequency is a variable parameter. It can be
...
\label{subsec:effects_edit_chorus}
   ...
   these different timbres by ourselves. \textsl{Yoshimi} therefore simply
   plays the sound, pitch modulated by an LFO, and adds this to the original
   sound.
...
\label{subsec:effects_edit_distortion}
   ...
   every wave that one can generate.
...
\label{subsubsec:effects_edit_phaser_circuit}
   ...
   We have no circuit diagram, just a picture of the results of the phaser effect
   in the form of a spectrogram.

\begin{figure}[H]
   \centering
   \includegraphics[scale=0.75]{zyn/effects/phaser-spectrogram.jpg}
   \caption{Phaser Circuit Spectrogram}
   \label{fig:phaser_circuit_spectrogram}
\end{figure}
   ...
   The spectrogram describes how this works on white noise. Light blue
   indicates that the frequency is not present at the current time, and dark
   blue indicates the opposite. The dark blue peaks appear if the delay time
   ...
   \textsl{Yoshimi} offers different types of phasers:
   ...
   \begin{itemize}
      ...
      This converts the triangle wave
...
%  TODO: Barber is deactivated, since PLFOtype is only 0 or 1?
   ...
   \begin{enumber}
      \item First, the LFO is generated.
         ...
         that define the LFO.
      \item \textbf{Phase} and \textbf{Depth} are applied afterwards in the
            usual way.
%           (TODO: I don’t understand the code here for the normal phase.)
            For the analog phaser (see the \textbf{Analog} check-box),
            \textbf{Phase} is not implemented yet.
      \item If \textbf{hyper} is set, then the LFO function is squared.
      \item Next, the input is used.
      \item The \textbf{Analog} setting
            decides whether the phaser is analog or "normal".
      \item \textbf{Pan} applies panning to the original input in every loop.
      \item Next, barber-pole phasing is applied (\textbf{Analog} only).
      \item \textbf{Fb} applies feedback. The last sound buffer element is (after
            phasing) multiplied by this value and then added to the current
            one. For the normal filter, the value is added before, and, for
            analog, after the first phasing stage.
      \item Next, based on the setting of \textbf{Stages},
            phasing stages are applied.
            The \textbf{dist} control sets the amount of
            distortion when applying the phasing stages. This
            setting has only effect for analog phasers.
      \item The feedback is applied next, for analog filtering.
      \item In the end, the \textbf{Substract} option
            inverts the signal, multiplying it by -1.
   \end{enumber}
...
%  TODO.
   ...
   Phaser Hyper.
   Checking this box sets the "hyper-sine" mode, whatever that is.

   Values: \texttt{Off*, On}

   \itempar{Analog}{phaser!analog}
   Phaser Analog.
   Checking this box emulates the "FET".  Field-effect transistor?
