...
\label{subsubsec:bottom_panel_controllers_midi_controls}
   ...
   There is a new very small window, accessed by right clicking on
   the 'controllers' button.

   This has just 4 controls that are MIDI-learnable:

   \begin{enumber}
      \item Modulation
      \item Expression
      \item Filter Q
      \item Filter Cutoff (Brightness)
   \end{enumber}

\begin{figure}[H]
   \centering 
   \includegraphics[scale=1.0]{1.5.0/MIDIcontrols.png}
...
\end{figure}
   ...
   correctly with it. An interesting effect was to have a sawtooth wave
   set up in \textbf{AddSynth} and the frequency LFO level turned up.
   Then Will learned \textbf{Modulation}, \textbf{Expression}, and
   \textbf{Filter Cutoff}, all on aftertouch. He then reduced their
   ranges in the MIDI learn window and the effect was very interesting.

   Because of the way this is implemented, these controls are also accessible via
   CLI direct access for both for read and write, and the knobs will respond to
   this, as well as to learned controls.
...
\end{figure}
   ...
   \begin{enumber}
      ...
      \item \textbf{Kit Edit}
      \item \textbf{Effects}
      \item \textbf{Rnd. Det.}, now a \textbf{Humanise} slider.
      \item \textbf{Close}
   \end{enumber}
...
\end{figure}
   ...
   \itempar{Humanise (Rnd. Det.)}{edit!rnd det}
