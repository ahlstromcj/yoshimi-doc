...
\label{subsec:bottom_panel_controls}
   ...
   This dropdown menu is new in \textsl{Yoshimi} V 1.7.1 and detirmines the way
   stereo sounds are tracked across, and their effect in mono.

   'Cut' tends to make the sound move away as it reaches the extreme left or
   right, and as it's name suggests seems to make it quieter in mono.

   'Default' is what we've been using up to now, and gives a fairly even reponse.

   'Boost' seems to make the sound move closer at the extremes, and, uniquely,
   leaves the volume unchanged in mono.

   Note that when listening on speakers, the greater the distance from the
   speakers, the closer one is to hearing a mono sound, so the setting of this
   control can have quite a profound effect. It is stored in patch sets.
   ...
   the identically numbered System Effect (in the panel above). Obviously if
   there is no system effect set then the control will do nothing.
...
%  \includegraphics[scale=1.0]{bottom-panel/controllers-dialog.jpg}
   \includegraphics[scale=1.0]{1.7.1/Controllers.png}
...
\label{subsubsec:bottom_panel_controllers_aftertouch}
   This is completely new in \textsl{Yoshimi V 1.7.1} and clicking on the button
   in Controllers will open the window below.

\begin{figure}[H]
   \centering
   \includegraphics[scale=0.7]{1.7.1/AfterTouch.png}
...
Both types of aftertouch are supported, Channel and Key. Many keyboards only have Channel aftertouch , but Key is becoming more common. Key is where the aftertouch behaviour only affects the note produced by the key being pressed harder.

Currently both forms support Filter Cutoff point, Filter Q, and Pitch Bend. Also note the extra checkboxes where these can be set to move the controls down instead of up. Volume is only available to Channel pressure.

You can have multiple actions such as Filter Q and Pitch Bend, but you can't have the same control on both Channel and Key.
...
%  \includegraphics[scale=1.0]{1.3.9/edit-instrument.png}
   \includegraphics[scale=1.0]{1.6.0/edit_instrument.png}
