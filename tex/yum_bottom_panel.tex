...
%     \item \textbf{Pan Reset Button}
   ...
   like that. The problem then was once re-loaded you had no idea what was
   ...
   If one holds down the \texttt{A} key, then left-clicks the
   \textbf{Edit} button, this brings up directly the ADDSynth editor,
   if it is enabled.
   However, if one uses the right mouse button, that will enable it and then
   open it.
   The same is true with \texttt{S} for SUBSynth and \texttt{P} for PADSynth.

   On the other hand, holding down the \texttt{K}
   key then clicking will open the Kit
   editor.
   The \texttt{E} key opens the part's Effects window.
...
%  \includegraphics[scale=1.0]{bottom-panel/controllers-dialog.jpg}
   \includegraphics[scale=1.0]{1.5.6/Controllers.png}
...
%  \includegraphics[scale=1.0]{1.3.9/edit-instrument.png}
   \includegraphics[scale=1.0]{1.5.7/edit-instrument.png}
...
\end{figure}
   ...
   information directly into the XML file.
