...
\begin{document}
\title{Yoshimi Advanced Reference Manual, v 2.2.0}
...
%   \includegraphics[scale=0.60]{2.0/Front.png}
   \includegraphics[scale=0.60]{2.2.0/Main.png}
\end{figure}
...
\label{subsubsec:new_features_2_2_0}
   ...
   Alteratively one can set new control in the PadSynth engine itself to
   ...
   This is most usefull when wanting to try changing something but with the
   ...
   this to the standard 120BPM. Now, howver there is a control so that it
...
\label{subsubsec:new_features_2_1_1}
   ...
   These controls be used with no MIDI source connected, and they can be
...
%  This will be given the next free ID.
...
\label{subsubsec:new_features_miscellany}
   ...
   to tell which version created the files, or whether they were created by
   \textsl{ZynAddSubFX}. If the new files are not compatible (such as config)
   the doctype and all other details are identfied as purely for
   \textsl{Yoshimi}.
%  However, see \sectionref{subsec:file_compatibility}.
...
\label{subsec:file_compatibility}
   ...
   both older \textsl{Yoshimi} versions will run correctly, and will do so on
   \textsl{ZynAddSubFX}. Obviously, completely new features will be lost to
   older versions but   othewise files will behave as before. This is
   especially relevant where exisiting files are loaded and then resaved.
...
\index{new!Yoshimi/ZynAddSubFX Warning}
   ...
   different approach, and some prameters have different types in their
   versions from 3.0 onwards. Therefore we now make a test for this and give
...
\begin{figure}[H]
   \centering
   \includegraphics[scale=1.0]{1.7.2/CompatibilityWarning.png}
...
\end{figure}
   ...
   here because it only appears for three or four seconds when one runs
...
\begin{figure}[H]
   \centering
   \includegraphics[scale=0.75]{2.0/DefaultWarn.png}
