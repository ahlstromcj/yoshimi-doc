...
%     \item \textbf{Select}
      \item \textbf{Hide Non Fatal Errors} (new)
      \item \textbf{Log XML headers} (new)
...
\end{figure}
   ...
   The default internal buffer size has been reduced from 1024 to 256.
   One gets better latency that way.
...
\end{figure}
   ...
   In general, 0 is probably the best setting for debugging only.
   ...
   This setting should stay at 3 if one is going to save large patchsets that
   will later be loaded while running. Uncompressing is \textsl{much}
...
\end{figure}
   ...
   This itme sends the information to the console window (or stderr) so that
...
\end{figure}
   ...
   so if you check ALSA for MIDI, it automatically unchecks JACK for MIDI.
   ...
   first attempt. The switches are mutually exclusive across JACK and ALSA,
   so if you check ALSA for audio, it automatically unchecks JACK for audio.
...
\end{figure}
   ...
   Note that both ALSA and JACK will support the 192000 rate, if the sound-card
   supports it.
...
% HAVE THESE ITEMS BEEN MOVED OR DELETED????
%
...
%  Values: \texttt{0-127, 0*}
   ...
   as it tells you exaclty what \textsl{Yoshimi} thinks it received.
...
\label{paragraph:menu_scales_keyboard_mapping}
   ...
   now stored in the new history file, and can be quickly reinstalled with a
