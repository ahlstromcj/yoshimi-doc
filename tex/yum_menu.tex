...
\begin{figure}[H]
   \centering
   \includegraphics[scale=0.75]{1.5.4/view_manual.png}
   \caption[Yoshimi Menu, Exit]{Yoshimi Menu}
   \label{fig:yoshimi_view_manual}
\end{figure}
...
%  (Alt-Y, Alt-I, Alt-P, and Alt-S) do not work (Yoshimi 1.3.5).
   ...
   \textsl{View Manual} is a new feature of \textsl{Yoshimi}.  It
   currently requires that a PDF viewer be installed.
...
%  \includegraphics[scale=1.0]{1.5.0/About_1-5-1.png}
   \includegraphics[scale=1.0]{1.5.9/About.png}
...
\end{figure}
    ...
    all those who have helped Yoshimi. Somtimes just a hint from these friends
    has been enough to pave the way for extensive improvements.
\begin{figure}[H]
   \centering
   \includegraphics[scale=1.0]{1.5.9/Contributors.png}
...
\label{subsubsec:menu_yoshimi_new_instance}
   ...
   by running a JACK session manager such as QJackCtl.
   ...
   The bank structure should be synchronised, but currently isn't.
...
\end{figure}
   ...
   will (to a small degree) represent wasted processor use.

   See \sectionref{sec:resource_usage_jack_alsa_buffer_sizes}, which
...
\end{figure}
   ...
   later be loaded while running. Uncompressing is \textsl{much} faster than file
   loading.
...
%  \includegraphics[scale=1.0]{1.5.0/console-window.png}
   \includegraphics[scale=1.0]{1.5.9/Console.png}
...
%  \includegraphics[scale=0.75]{1.5.0/SettingsSwitches.png}
   \includegraphics[scale=0.75]{1.5.8/SettingsSwitches.png}
...
\end{figure}
   ...
   the command line at the same time. This means that Yoshimi can only be closed by
   sending a special MIDI message (CC 99, 68, CC 98, 68). Alternatively one could
   send a system shutdown message.

   \itempar{Enable Auto Instance}{Switches!Enable Auto Instance}
	If checked, then, if any other instances were open at the time you close the
	main instance, then these other instances
   will be re-opened next time one runs \textsl{Yoshimi}.
...
%  \includegraphics[scale=0.75]{1.5.4/SettingsMidi.png}
   \includegraphics[scale=0.75]{1.5.7/SettingsMidi.png}
...
\end{figure}
   ...
   if you want to use a programmable hardware controller.
...
\end{figure}
   ...
   exceptional is it will \textsl{not} appear in the history list, and so
